...
**携帯版タッチ操作・ボトムシートUI・メニュー修正** — ズームコントロール・オーバーレイ文字・信頼度・読み順ボタンがタッチに反応しなかった問題を修正。画像補正パネルをボトムシート方式に変更（画面下部からスライドアップし、画像を確認しながら補正可能）。保存メニューもボトムシート化（ツールバーoverflowによるクリップを解消）。メニューを左スライドに変更。Upload image/PDFボタンとフッターの重なりを修正。ドロップゾーン超コンパクト化。テキストエディタ縦方向領域を拡大（80vh）。
